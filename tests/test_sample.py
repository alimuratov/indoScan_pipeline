...
if os.environ.get("RUN_NUMPY_TESTS") != "1":
    pytest.skip(
        "Set RUN_NUMPY_TESTS=1 to run synthetic tests that require NumPy/Open3D",
...
def _to_o3d(points: np.ndarray, colors: Optional[np.ndarray] = None):
    ...
    if colors is not None and colors.shape == (points.shape[0], 3):
        pc.colors = o3d.utility.Vector3dVector(
            colors.astype(float, copy=False))
    return pc
...
def generate_bell_pothole_points(
    ...
    cols = np.tile(np.array([[1.0, 0.0, 0.0]],
                   dtype=np.float64), (n_points, 1))
    return pts, cols
...
def test_sample(tmp_path):
    ...
    points2, outliers_idx = inject_outliers(
        points, num_outliers=20, z_delta=0.15, seed=42)
    ...
    filtered_pts, _, keep_mask = filter_pothole_candidates(
        pothole_pts, None, ground_o3d, tmp_path=tmp_path, k=10, radius=2.0, max_below=2.0)
